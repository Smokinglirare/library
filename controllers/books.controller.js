...
async function addBook(req, res) {
  ...
  if (!name || !author) {
    return res
      .status(400)
      .json({ error: "You have to insert both name and author" });
  }
...
async function updateBook(req, res) {
  ...
  if (!req.body.name || !req.body.author) {
    return res
      .status(400)
      .json({ error: "You need to insert both name and author of book" });
  }
...
async function updateDetailOfBook(req, res) {
  ...
  if (!req.body.name && !req.body.author) {
    return res
      .status(400)
      .json({ error: "You have to change either name or author." });
  }
